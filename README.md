## User Documentation for Bistro

This directory will contain the user and feature documentation for Bistro. The documentation will be hosted on GitHub pages.
...
1. Ensure that you are in the same directory where this `README.md` and the `Gemfile` file exists (e.g., it could be in `/docs` on `master`, in the root of a `gh-pages` branch, etc). The below RubyGems commands, etc. must be run from there.
