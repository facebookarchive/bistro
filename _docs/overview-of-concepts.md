...
Shell-command can write their status to their `argv[2]` as one of the above strings, or as a JSON string with extra metadata (see [Task execution](https://facebook.github.io/bistro/docs/task-execution)). A task that fails to emit a status is taken to be in ["error_backoff", unless it was killed by Bistro](https://github.com/facebook/bistro/blob/master/bistro/processes/TaskSubprocessQueue.cpp#L64). In rare cases, you might use other [status codes not listed above](https://github.com/facebook/bistro/blob/master/bistro/statuses/TaskStatus.h).
