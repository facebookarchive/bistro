  ...
  *w3.machineLock()->port() = 123;
  ...
  auto check_all_workers_fn = [&](std::vector<cpp2::BistroWorker> workers) {
    ...
    for (const auto& w : workers) {
      EXPECT_EQ(
          *w.id(),
          *r.getWorker(*w.shard())->getBistroWorker().id());
  ...
  // Try and fail to replace w1 with another worker before w1 is lost.
  {
    auto check_worker_pools_unchanged_fn = [&]() {
      EXPECT_EQ(
          IDSet({*w1.id(), *w2.id()}), dredgeHostPool(r, "host1"));
      EXPECT_EQ(IDSet({*w3.id()}), dredgeHostPool(r, "host2"));
      check_all_workers_fn(std::vector<cpp2::BistroWorker>({w1, w2, w3}));
    };
    ...
    *w1new.machineLock()->port() = 789;
    ...
    // w1new's heartbeat was rejected, and we ask it to commit suicide
    {
      ...
      EXPECT_EQ(
          *w1new.id(), *update2.suicideWorkers().begin()->second.id());
  ...
  // Successfully move a worker from one host to another
  {
    ...
    *w1new.machineLock()->port() = 789;
    ASSERT_NE(*w1.id(), *w1new.id());
    {
      ...
      EXPECT_EQ(1, update2.suicideWorkers().size());
      EXPECT_EQ(
          *w1.id(), *update2.suicideWorkers().begin()->second.id());
      EXPECT_EQ(1, update2.newWorkers().size());
      EXPECT_EQ(
          *w1new.id(), *update2.newWorkers().begin()->second.id());
    }
    EXPECT_EQ(IDSet({*w2.id()}), dredgeHostPool(r, "host1"));
    EXPECT_EQ(
        IDSet({*w1new.id(), *w3.id()}), dredgeHostPool(r, "host2"));
...
void addWorker(
  ...
  EXPECT_NO_PCRE_MATCH(no_consensus_re, fd->readIncremental());
  {
    ...
    EXPECT_FALSE(
        r->getWorker(*w.shard())->indirectWorkerSetID().has_value());
    r->updateState(&update);
    EXPECT_PCRE_MATCH(".*Waiting for all.*", update.initialWaitMessage());
  }
...
TEST_F(TestRemoteWorkersInitialWait, AchieveAndMaintainWorkerSetConsensus) {
  ...
  addWorker(&stderr, &r, w2, id12, {id12}, workerSetID(r, 1, *id2.hash()));
  addWorker(
...
void addAndUpdateWorker(
  ...
  EXPECT_EQ(
      num_workers, *r->nonMustDieWorkerSetID().hash()->numWorkers());
  ...
  EXPECT_FALSE(
      r->getWorker(*w.shard())->firstAssociatedWorkerSetID().has_value());
  {
    RemoteWorkerUpdate update(RemoteWorkerUpdate::UNIT_TEST_TIME, 1);
...
TEST_F(TestRemoteWorkersInitialWait, HistoryAndWorkerSetIDPropagation) {
...

  {
    RemoteWorkerUpdate update(RemoteWorkerUpdate::UNIT_TEST_TIME, 1);
    EXPECT_EQ(
        wid123, *r.processHeartbeat(&update, w2, wid123)->workerSetID());
  }
...

  {
    RemoteWorkerUpdate update(RemoteWorkerUpdate::UNIT_TEST_TIME, 1);
    EXPECT_EQ(
        wid1234, *r.processHeartbeat(&update, w2, wid1234)->workerSetID());
  }
...

  {
    RemoteWorkerUpdate update(RemoteWorkerUpdate::UNIT_TEST_TIME, 1);
    EXPECT_EQ(
        wid234, *r.processHeartbeat(&update, w3, wid234)->workerSetID());
  }
  ...
  history.insert( // History got pruned: w2 and w4 are keeping v4 alive.
      {4,
       {folly::none,
        {*w1.shard(), *w2.shard(), *w3.shard(), *w4.shard()}}});
  updateWorkers(
    &r,
  ...
  *w1a.id()->startTime() =
      *w1.id()->startTime() + 1; // This is a **newer** worker.
  CHECK_GT(
      *w1a.id()->startTime(),
      *w1.id()->startTime()); // Fail loudly if we overflow.
  ...
  );
  {
    ...
    EXPECT_EQ(
        wid1a23, *r.processHeartbeat(&update, w4, wid1a234)->workerSetID());
  }
...

  {
    RemoteWorkerUpdate update(RemoteWorkerUpdate::UNIT_TEST_TIME, 1);
    EXPECT_EQ(
        wid1a23, *r.processHeartbeat(&update, w1a, wid1a23)->workerSetID());
  }
